...
For those who have not dared to play, Wordle is a game where the user gives players six chances to to guess a random 5 letter word.
- Correct letters in the right place == Green Box 
- Correct letters in the wrong place == Yellow Box
- Wrong Letters == Grey Box

This effective yet succinct C++ program, to simulate Wordle in the command line, has been written, compiled and run on
Windows 10, through Dev C++ and Visual Studio Code. (I attempted to compile and run on Mac OSX but for could not solve the color scheme 
issue with the letters, even using ANSI codes for some reason i could not resolve. I found a interesting way using <windows.h> on stackexchange
...
I have extensively commented every block of code within the .cpp file and header file as best as I can, to give the user a clearer understanding
of how the file functions when it is compiled and run. All the files are grouped accordingly so selecting Game Files as the working directory
should be simple. The text file contains a selection of 10 tricky words but this can easily be expanded by the user, with appropriate adjustment 
mentioned in the comments witin the code. For simple testing purposes when running the program, 10 words was enough although even with just 10, guessing 
one was hard enough sometimes!!
